...
mkdir -p themes
ln -sf ../.. themes/adrian-hugo 2>/dev/null || true
...
hugo --config config.toml,hugo-temp.toml --gc --minify -d public

# If that fails, try with themesDir as backup
if [ $? -ne 0 ]; then
    rm -f hugo-temp.toml
    hugo --config config.toml --gc --minify --themesDir ../.. -d public --ignoreCache
fi
